...
void merge(Register *registers, size_t left, size_t middle, size_t right) {
  ...
  Register *left_reigsters = (Register *)malloc(sizeof(Register) * left_sz);
  Register *right_registers = (Register *)malloc(sizeof(Register) * right_sz);

  for (size_t i = 0; i < left_sz; i++)
    left_reigsters[i] = registers[i + left];
  ...
  for (size_t i = left; i <= right; i++) {
    if (right_index == right_sz ||
        left_reigsters[left_index].id < right_registers[right_index].id) {
      registers[i] = left_reigsters[left_index];
      left_index++;
    } else {
